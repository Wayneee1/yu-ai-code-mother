...
flowchart LR
    ...
    AppAPI --> Auth[用户与权限]
    AppAPI --> AppService[应用服务]
    AppService --> Router[AI 生成类型路由]
    Router --> AI[LangChain4j AI Service]
    AI --> Tools[文件工具调用]
    AI --> Parser[代码解析与保存]
    Parser --> Output[生成项目目录]
    AppService --> Deploy[部署与下载]
    Deploy --> Screenshot[Selenium 截图]
...
```mermaid
flowchart TD
    Start([Start]) --> ImageCollector[图片资源收集]
    ImageCollector --> PromptEnhancer[Prompt 增强]
    PromptEnhancer --> Router[生成类型路由]
    Router --> CodeGenerator[代码生成]
    CodeGenerator --> QualityCheck[代码质量检查]
    QualityCheck -->|通过且需要构建| ProjectBuilder[项目构建]
    QualityCheck -->|通过且无需构建| End([End])
    QualityCheck -->|未通过| CodeGenerator
    ProjectBuilder --> End
```
...
```mermaid
flowchart LR
    GatewayClient[前端/调用方] --> AppService[yu-ai-code-app]
